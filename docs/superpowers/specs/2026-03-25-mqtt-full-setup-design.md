...
- `xiaozhi.mqtt.broker-url: tcp://emqx:1883`（服务端连接，Docker 内部网络）
- `xiaozhi.mqtt.device-endpoint: <宿主机IP>:1883`（设备连接，外部可达地址，**新增配置项**）
- 配置 `device-username` / `device-password` 用于设备认证

> **broker-url 与 device-endpoint 分离**：`broker-url` 是服务端连接 EMQX 的地址（Docker 内部网络名 `emqx:1883`），ESP32 设备无法访问。`device-endpoint` 是设备连接 EMQX 的外部可达地址（开发环境为宿主机 IP，生产环境为公网域名如 `mqtt.example.com:8883`）。`MqttConfigGenerator` 优先使用 `device-endpoint` 生成 OTA 配置，未配置时回退到 `broker-url`（去掉协议前缀）。需要在 `MqttProperties` 中新增 `deviceEndpoint` 字段，并修改 `MqttConfigGenerator.generateMqttConfig()` 的 endpoint 取值逻辑。
...
```
加密（服务端发送）：
  1. 复制初始 nonce 模板（16 字节，其中 [0]=0x01, [1]=0x00, [4:7]=SSRC 已固定）
  2. 在 nonce 中写入本包元数据：payload_len(字节2-3), timestamp(字节8-11), sequence(字节12-15)
     注意：不修改 [0]、[1]、[4:7]，它们保持初始值
...
- 初始 Nonce：16 字节，hello 响应中以 32 字符 hex 下发，作为 nonce 模板。**不能全随机**，特定字节有固定含义：

```
初始 nonce 结构（16 字节）：
字节 [0]   = 0x01    （type 标记，ESP32 接收端校验 data[0]==0x01，不等则丢弃）
字节 [1]   = 0x00    （flags，保留）
字节 [2:3] = 随机    （每包被 payload_len 覆盖）
字节 [4:7] = SSRC    （同步源ID，标识 session，服务端用于路由，整个会话不变）
字节 [8:15]= 随机    （每包被 timestamp 和 sequence 覆盖）
```

- ESP32 发送时只修改 nonce[2:3]、[8:11]、[12:15]，**[0]、[1]、[4:7] 保留初始值**
- 服务端收到设备包后，从 nonce[4:7] 取出 SSRC 路由到对应 session
- 服务端发送时同样遵循此结构，nonce[0]=0x01 确保 ESP32 接收端校验通过
...
1. **hello 消息**（建立对话通道，参照 `WebSocketHandler.afterConnectionEstablished` + `handleHelloMessage` 的等价逻辑）：
   - 从 topic 解析 userId 和 deviceId
   - 分配 session_id 和 SSRC
   - 随机生成 AES-128 密钥，构造初始 nonce（byte[0]=0x01, [1]=0x00, [4:7]=SSRC, 其余随机）
   - 创建 `UdpSessionContext` 和 `MqttUdpSession`
   - **调用 `messageHandler.afterConnection(session, deviceId)`** —— 复用现有设备初始化逻辑：
     - 注册 session 到 `SessionManager`
     - 查库获取 `SysDevice`（不存在则创建）
     - 如果设备已绑定（roleId != null），调用 `boundDeviceInitializer.initializeDevice()`（设置工具持有者、构建角色人设、初始化 AEC）
   - 通过 MQTT command topic 回复 hello 响应（含 UDP server/port/key/nonce）

2. **listen / abort / goodbye / iot / mcp 等消息**：
   - 从 topic 或 JSON 中获取 deviceId，查找对应 session
   - 将 JSON 解析为对应的 Message 子类（`ListenMessage`、`AbortMessage` 等）
   - **委托给现有 `messageHandler.handleMessage(msg, sessionId)` 处理**，复用已有的消息路由逻辑（Pattern Matching switch 分支）
   - goodbye 消息还需额外调用 `messageHandler.afterConnectionClosed(sessionId)` 清理设备状态
...
xiaozhi:
  udp:
    enabled: true
    port: 8888              # UDP 音频端口
    external-ip: ""         # 服务端外部可达 IP（hello 响应中的 udp.server）
    buffer-size: 4096       # 接收缓冲区大小
```

> **udp.external-ip**：hello 响应中需要告诉设备 UDP 服务的外部可达地址（`udp.server` 字段）。Docker 开发环境填宿主机局域网 IP，云部署填公网 IP。如果留空，服务端应自动检测本机 IP（通过网络接口枚举），但自动检测在 Docker 容器中可能返回容器内部 IP，因此生产环境**必须显式配置**。此配置项在 `UdpProperties` 中定义，`MqttDialogueHandler` 构造 hello 响应时使用。
...
```
音频上行：
  设备 → UDP(加密Opus) → UdpAudioServer → 按SSRC查找session → AES解密
  → dialogueService.processAudioData(mqttUdpSession, opusData)
  → Opus解码 → VAD → STT → LLM → TTS → Opus编码
  → mqttUdpSession.sendBinaryMessage(opusData)
  → UdpAudioServer AES加密 → UDP直发设备

控制消息：
  设备 → MQTT(status topic) → MqttDialogueHandler
    hello → 创建session + 设备初始化 + 回复UDP参数
    listen/abort/goodbye/iot/mcp → 委托 messageHandler.handleMessage()
  服务端 → MQTT(command topic) → 设备
    hello响应 / STT结果 / TTS指令 等
```
...
| `communication/mqtt/MqttProperties.java` | 新增 `deviceEndpoint` 字段（设备连接的外部可达 MQTT 地址） |
| `communication/mqtt/MqttConfigGenerator.java` | endpoint 取值优先使用 `deviceEndpoint`，未配置时回退到 `brokerUrl` |
...
实时音频对延迟敏感，不适合重传。策略与 ESP32 固件行为对齐（mqtt_protocol.cc 第 259-265 行）：
- **服务端不做 jitter buffer**，收到即处理（与 WebSocket 方案一致）
- **丢包**：由 Opus 解码器内置 PLC（Packet Loss Concealment）补偿
- **旧包丢弃**：如果收到的 `sequence < remote_sequence_`（小于已处理的最大序列号），直接丢弃
- **非连续包**：如果 `sequence != remote_sequence_ + 1`（不是严格下一个），记录警告日志但仍正常处理
- **sequence 更新**：处理完成后 `remote_sequence_ = sequence`
